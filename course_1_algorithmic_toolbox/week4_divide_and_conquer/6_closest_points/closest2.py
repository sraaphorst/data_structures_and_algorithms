...
def brute_force(xs, ys):
    ...
    dist, p, q = min([(distance(p, q), p, q) for p in points for q in points if p != q])
...
class TestClosest(unittest.TestCase):
    def test1(self):
        ...
        self.assertEquals(minimum_distance(xs, ys), math.sqrt(2))
    def test_random(self):
        num_points = randrange(0, 10**3)
        xs = [randrange(-10**9, 10**9) for _ in range(num_points)]
        ys = [randrange(-10**9, 10**9) for _ in range(num_points)]
        self.assertEquals(minimum_distance(xs, ys), brute_force(xs, ys))
